...
## Version 5.0
**Previous version:** prd_v4.md
**Date:** April 2026

### Changelog from v4
- Issue resolutions documented: all 7 open issues from post-launch review resolved
- feed_type column added to sources schema (news | blog)
- Blog feed pipeline spec added: content:encoded used directly, video item detection, HTML entity decoding
- Video items in newsletter: within Quick Hits slot, blog sources only, one max per newsletter
- Multi-team subscriptions: schema confirmed as UNIQUE(email, team_id), unsubscribe per team, engagement tracking per subscription
- Pipeline health status bar added to admin spec (persistent, all tabs)
- Add Source inline form added to Source Queue tab spec
- CLAUDE.md rules added: sanitize Claude strings before DB writes, exclude /api/* from vercel.json SPA rewrite
- Phase 3 selection clarified: two-stage process (threshold filter + Claude selectAndRankArticles)
- Phase boundary documented: Phase 3 = selection, Phase 8 = scoring
...
```
Constraint: UNIQUE(email, team_id) — one row per email+team combination.
One subscriber can follow multiple teams — each gets its own row and its own daily email.
Unsubscribe is per-row (per team) — clicking unsubscribe in a Seahawks email
only deactivates the Seahawks subscription, not any other team subscriptions.
Engagement tracking is per-row — open rate, satisfaction, churn all measured per (email, team_id).
...
feed_type (news | blog — default news)
...
**feed_type behaviour:**
- `news`: pipeline fetches full article body via HTTP scrape — standard path
- `blog`: pipeline reads `content:encoded` from RSS feed directly — no HTTP scrape
  Blog feeds mix article posts with video embeds. Pipeline classifies each item:
  - Text item (word_count >= 200 after decoding): treated as article, scored normally
  - Video item (content is empty or YouTube embed only): no Claude call, no word-count gate,
    rendered as Watch item in newsletter if selected
  - HTML entities decoded with he.decode() before any Claude call or DB write
...
### prompt_examples
```
id, team_id (FK → teams, NULLABLE — null means applies to all teams)
article_title, decision (accept | reject)
example_type (primary | ensemble | not_relevant | non_nfl)
reasoning
created_at
```
Used to inject few-shot examples into Claude relevance and scoring prompts.
Admin can add examples via "Correct Claude" button in Article Logs tab.
Global examples (team_id = NULL) apply to all teams.
Fetch up to 8 examples per team per call — mix of team-specific and global.

### team_context
```
team_id (PK, FK → teams)
season_phase (offseason | preseason | regular_season | playoffs | draft_period)
key_storylines (text — e.g. "Price drafted at #32, edge rusher depth unresolved")
deprioritize_topics (text — e.g. "preseason stats, cap space speculation")
updated_at
```
Admin updates weekly via Team Context editor in admin dashboard.
Injected into every Claude relevance and summarisation call for this team.
Fetched once per team at pipeline start — not per article.
...
CREATE INDEX idx_prompt_examples_team ON prompt_examples(team_id);
```
...
Seed list:
- Pro Football Talk (`https://profootballtalk.nbcsports.com/feed/`) — open access, high volume
- CBS Sports NFL (`https://www.cbssports.com/nfl/feed/rss/`) — open access, broad coverage
- NFL Trade Rumors (`https://nfltraderumors.co/feed/`) — open access, transaction-focused

**Removed from v3:**
- ESPN NFL — paywalled for unauthenticated scrapers, articles fail word_count quality gate
- The Athletic NFL — paywalled (NYT), same issue as ESPN

**Rule:** General sources must be fully accessible to unauthenticated HTTP scrapers.
A source that returns < 200 words after scraping fails the quality gate on every article
and provides no value. Paywalled sources must not be seeded as general sources.
...
- For each source: parse RSS feed via `rss-parser` → extract article list (title, link, pubDate, description, content:encoded)
- Apply 3-article-per-source cap AFTER full fetch — keep 3 most recent per source

**Body fetching — branches on feed_type:**

`feed_type = news` (standard path):
- Fetch full article body via HTTP scrape of article URL
- RSS excerpt is never sufficient — always scrape full page
- Discard if word_count < 200 after scraping

`feed_type = blog` (blog path):
- Use content:encoded from RSS feed directly — do NOT fetch article URL
- Decode HTML entities using he.decode() before processing
- Classify item type:
  - Text item: content:encoded has >= 200 words of readable text after stripping tags
  - Video item: content:encoded is empty, or contains only a YouTube/video embed with no text
- Text items proceed to relevance check normally
- Video items: skip quality gate and Claude calls — mark as category = video_link
  Store: title, original_url, youtube_url (extracted from embed), source_id
- Discard blog items with < 200 words that are not video items (low-content posts)

**All sources:**
- HTML entities decoded before any Claude call or DB write
- All Claude-generated strings sanitized before DB write — strip control characters (\n, \r, \t, null bytes)
...
The relevance prompt injects two context blocks before the question:
1. team_context — current season phase and key storylines for this team
2. prompt_examples — last 8 accepted/rejected examples for this team

```
[CONTEXT BLOCK — injected from team_context table]
Current context for [CITY] [TEAM NAME]:
Season phase: [season_phase]
Key storylines: [key_storylines]
Deprioritise: [deprioritize_topics]

[EXAMPLES BLOCK — injected from prompt_examples table]
Past decisions for [CITY] [TEAM NAME]:
✅ ACCEPTED: "[example title]" — [reasoning]
❌ REJECTED: "[example title]" — [reasoning]
(up to 8 examples, mix of team-specific and global)

[QUESTION]
Does this article contain meaningful content about the [CITY] [TEAM NAME]?

Answer YES (relevant: true) in either of these cases:
1. PRIMARY: The article is substantially about this team, their players,
   coaches, or front office decisions.
2. ENSEMBLE: The article covers all 32 NFL teams or a large subset —
   including any of: draft grades, power rankings, season previews,
   trade deadline roundups, "every team's biggest need/question",
   mock drafts, offseason grades. These always contain a dedicated
   section for every team including [TEAM NAME].

Answer NO (relevant: false) only if the team is not mentioned at all,
or gets only a passing single-sentence reference with no real content.

If ENSEMBLE: set is_ensemble = true in your response.

Article title: [title]
Article body excerpt: [first 1000 chars of raw_content]

Reply JSON only: { "relevant": boolean, "confidence": 0-100,
"reasoning": "one sentence", "is_ensemble": boolean }
```
...
- If Claude API call fails or JSON parse fails: default to relevant=false
- Articles that fail: log with rejection_reason = not_relevant
- Articles that fail: do NOT proceed to Step 3 — discarded immediately
- is_ensemble flag passed to Step 6 summarisation — changes summarisation prompt

**Ensemble summarisation (Step 6 — when is_ensemble = true):**
```
This is an ensemble article covering all 32 NFL teams.
Extract and summarise ONLY the section about [CITY] [TEAM NAME].
Ignore all other teams entirely.
Write 2-3 sentences covering only what this article says about [TEAM NAME].
Do not mention other teams' grades or outcomes.
Source: [article title and author]
```
...
Two-stage process: threshold filter then Claude editorial selection.

**Stage 1 — Threshold filter (deterministic):**
- All non-injury articles with composite_score >= 65 pass to Stage 2
- Injury-category articles pulled out separately — go to Injury Report
- Source diversity applied: if all passing articles from same source, force one from a different source

**Stage 2 — Claude editorial selection (`selectAndRankArticles`):**
Runs only when > 5 non-injury articles pass Stage 1. Skipped when <= 5 pass — composite_score order used directly.

```
You are selecting articles for a daily NFL newsletter for [TEAM NAME] fans.
From the following articles that passed quality and relevance checks,
select the best 5 for the newsletter. Rank them 1-5.

Criteria:
- News value: how significant is this story for a fan today?
- Variety: avoid redundancy — do not select two articles covering the same story
- Recency: prefer today's news over yesterday's
- Team relevance: prefer articles where [TEAM NAME] is the primary subject

Articles (title | category | composite_score | source):
[list]

Reply JSON only: { "selected": ["id1",...,"id5"], "reasoning": "one sentence" }
```

Falls back to composite_score order if Claude call fails — never blocks newsletter assembly.
Position 1 = Lead Story. Positions 2-5 = Quick Hits.

**Phase boundary — important:**
- Phase 3: Claude touches SELECTION — picks best 5 from threshold-passing articles
- Phase 8: Claude touches SCORING — replaces category-derived scores with 4-dimension scoring
These are distinct responsibilities. Phase 8 changes what passes the threshold. Phase 3 changes what gets picked from those that pass.

**Stat of the Day:**
- Team-specific stat from today's articles first
- NFL-wide contextual stat as fallback
- Omit entirely if nothing verifiable exists — never fabricate
- Stat must reference a real fetched article_id
...
2. **Quick Hits** — up to 4 items: text articles (headline + 2-sentence summary) and/or one video item
...
**Quick Hits slot rules:**
- Text articles fill slots first (scored, summarised, ranked)
- One video item allowed per newsletter — fills a remaining slot only if < 4 text articles exist
- Video items come from blog sources only — general news sources never produce video items
- Video item renders as: 📺 [Title] → Watch (link to YouTube/video URL)
- No summary, no score, no Claude call for video items
...
- [ ] news feed articles: full body fetched via HTTP scrape
- [ ] blog feed articles: content:encoded used directly — no HTTP scrape
- [ ] HTML entities decoded before any Claude call or DB write
- [ ] Video items detected from blog feeds — no Claude call, no word-count gate
- [ ] Articles with word_count < 200 discarded at quality gate (text items only)
- [ ] All Claude-generated strings sanitized before DB write
...
- [ ] team_context fetched once per team at pipeline start
- [ ] prompt_examples fetched once per team at pipeline start (up to 8)
- [ ] checkTeamRelevance prompt includes team_context and prompt_examples blocks
- [ ] checkTeamRelevance accepts both PRIMARY and ENSEMBLE article types
- [ ] Ensemble articles flagged with is_ensemble = true for summarisation step
...
- [ ] Stage 1: threshold filter correctly separates injury and non-injury articles
- [ ] Stage 2: selectAndRankArticles Claude call runs when > 5 non-injury articles compete
- [ ] Stage 2: falls back to composite_score order when Claude call fails
- [ ] Stage 2 skipped when <= 5 articles pass — composite_score order used directly
- [ ] Lead story = position 1, Quick Hits = positions 2-5
- [ ] Video items from blog sources fill remaining Quick Hit slots only — never displace text articles
- [ ] Maximum one video item per newsletter
- [ ] Video items rendered as title + watch link — no summary
...
**5.0 — New Admin Capabilities (v4 additions)**

**"Correct Claude" button — Article Logs tab**
On every article row in Article Logs, add a "Correct Claude" action:
- For rejected articles: "Should have been accepted" → saves as prompt_examples row (decision=accept)
- For accepted articles: "Should have been rejected" → saves as prompt_examples row (decision=reject)
- Admin must provide a short reasoning (pre-filled from article's existing rejection/selection reasoning)
- example_type inferred from article category or admin selection
- Saved examples immediately available in next pipeline run

**Team Context editor — Subscribers tab**
- Simple form per team: season_phase dropdown + key_storylines text + deprioritize_topics text
- Updated by admin weekly or after major team news
- Changes take effect on next pipeline run

**5.1 — Pipeline Health Status Bar**
Persistent bar pinned at top of every admin tab — always visible regardless of active tab.

```
🟢 Last run: Today 6:14 AM · Seattle Seahawks · 7 articles selected · Newsletter sent
🔴 Last run: Today 6:02 AM · Seattle Seahawks · Failed — JSON parse error in scraping
```

- Reads from pipeline_runs table — most recent row per active team
- Green dot: status = completed AND newsletter sent
- Red dot: status = failed OR no newsletter sent
- Shows: timestamp, team name, articles selected count, send status or error message (truncated 60 chars)
- Updates on page load — not real-time
- Alerts and thresholds deferred to a later phase

**5.2 — Source Queue Tab**
- All pending + flagged sources
- Source type badge: general | team-specific | user-submitted | feed type badge (news | blog)
...
**Add Source inline form** (in Source Queue tab header):
```
[Add source ▾]  ← button expands inline form below header

RSS feed URL: [________________________]
Scope:  [● Team-specific  ○ General (all teams)]
Feed type: [● News  ○ Blog]
Team: [Seattle Seahawks ▾]  ← only shown when team-specific selected

[Validate & Add]  [Cancel]

✓ Valid RSS feed · Seahawks relevance: 94% · Added as pending   ← inline result
```
- "Validate & Add" calls `/api/validate-source` and shows result inline
- Source appears in queue immediately after validation
- Team dropdown only visible when Scope = Team-specific
- General sources: no team required, team_id = NULL

**5.3 — Content Preview Tab**
...
- [ ] "Correct Claude" button on Article Logs saves to prompt_examples table
- [ ] Team Context editor saves to team_context table
- [ ] Changes to prompt_examples and team_context visible in next pipeline run
- [ ] Pipeline health status bar visible on all tabs with correct green/red state
- [ ] Status bar shows last run timestamp, team, article count, send status
- [ ] Add Source inline form expands in Source Queue header
- [ ] Add Source form calls /api/validate-source and shows result inline
- [ ] Source appears in queue after successful validation
- [ ] feed_type badge (news | blog) visible on source rows
- [ ] Source Queue shows all pending + flagged with inline validation detail
- [ ] Content Preview shows today's newsletter with scores
- [ ] Subscribers tab shows metrics computed from raw newsletter_sends and newsletter_metrics tables
...
- [ ] Design system tokens applied consistently across all tabs
...
**8.1b — Prompt Intelligence (from Phase 3 — now richer in Phase 8)**
Phase 8 scoring and summarisation prompts also include:
- prompt_examples: up to 8 curated examples from admin corrections — now inform scoring too
- team_context: season phase + current storylines — now used to weight significance scoring
- Combined with recent article headlines, Claude has full awareness of team situation
