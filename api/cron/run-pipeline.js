...
// src/lib/utils/sanitizeString.ts
var sanitizeForDb = (str2) => {
  if (!str2) return str2 ?? "";
  return str2.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "").replace(/\r\n/g, " ").replace(/[\r\n]/g, " ").replace(/\t/g, " ").trim();
};
...
If this is a league-wide roundup (e.g. draft grades for all 32 teams, power rankings), extract and summarize ONLY the ${params.teamDisplayName} section \u2014 ignore all other teams.
...
var COMPOSITE_BY_CATEGORY = {
  ...
  general: 50,
  video_link: 0
  // video items are not scored — they fill slots only when text < 4
};
...
var cleanBlogContent = (html) => {
  ...
  return import_he.default.decode(stripped).replace(/\s+/g, " ").trim();
};
...
var upsertArticlesAndInsertScoreLogs = async (supabase, params) => {
  ...
  const logsResolved = params.logs.map((log2) => ({
    ...
    headline: sanitizeForDb(log2.headline),
    selection_reasoning: log2.selection_reasoning ? sanitizeForDb(log2.selection_reasoning) : null,
    rejection_reason: log2.rejection_reason ? sanitizeForDb(log2.rejection_reason) : null
  }));
...
var buildVideoHit = (a, teamColor, isLast) => {
...
var runTeamPipeline = async (teamId) => {
  ...
  try {
    ...
    const selectedVideo = quick.length < 4 && videoCandidates.length > 0 ? videoCandidates[0] ?? null : null;
    const toSummarize = [];
