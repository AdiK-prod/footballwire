...
var deactivateSubscriber = async (subscriberId, client) => {
  const supabase = withClient(client);
  const { error } = await supabase.from("subscribers").update({ is_active: false }).eq("id", subscriberId);
  if (error) {
...
var handleUnsubscribeRequest = async (request) => {
  try {
    const url = new URL(request.url);
    const subscriberId = parseIntParam(url.searchParams.get("sid"));
    const sig = url.searchParams.get("sig") ?? "";
    if (!subscriberId || !verifySignature(config.unsubscribeSecret, subscriberPayload(subscriberId), sig)) {
...
async function handler(req, res) {
  const webRes = await handleUnsubscribeRequest(toRequest(req));
